...
scene.addEventListener(scene.Events.afterLoadScene, (sceneId, loadMode) => {
  ...
  if (sceneId == outsideTwinsenHouseSceneId) {
    ...
    mage.setEntity(15); // Meteomage entity (we can look entities, bodies and animation numbers in the LBAArchitect application).

    const bodies = mage.getAllBodies();
    console.log("This Entity bodies count and contents:", bodies.length, bodies);
